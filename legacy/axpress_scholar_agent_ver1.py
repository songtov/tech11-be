...
class AXPressScholarAgent:
    ...
    def __init__(self, download_dir: str = "downloaded_papers"):
        ...
        self.arxiv_categories = {
            ...
            "금융": [
                ...
                "q-fin.CP",
                "econ.GN",
            ],  # General Finance, Computational Finance
            "CLOUD": [
                "cs.DC",
                "cs.DS",
                "cs.SE",
            ],  # Distributed Computing, Data Structures, Software Engineering
    ...
    def fetch_highly_cited_papers(self, domain: str) -> List[Paper]:
        ...
        logger.info(
            f"도메인 '{domain}'에서 고인용 논문 검색 시작 (Semantic Scholar + arXiv)"
        )
        ...
        try:
            ...
            if not journals:
                logger.warning(
                    f"도메인 '{domain}'에 대한 저널 정보가 없습니다. 기본 검색을 수행합니다."
                )
            ...
            if not all_papers:
                logger.warning(
                    "Semantic Scholar에서 논문을 찾을 수 없습니다. 기본 검색을 수행합니다."
                )
                return self.fetch_papers(domain)
            ...
            if not recent_papers:
                logger.warning(
                    "2년 내 논문을 찾을 수 없습니다. 기본 검색을 수행합니다."
                )
                return self.fetch_papers(domain)
            ...
            logger.info(
                f"2년 내 인용수 기준 상위 {len(selected_papers)}편의 논문을 찾았습니다."
            )
    ...
    def display_papers(self, papers: List[Paper]) -> None:
        ...
        for i, paper in enumerate(papers, 1):
            print(f"\n{i}. {paper.title}")
            print(
                f"   저자: {', '.join(paper.authors[:3])}{' 외' if len(paper.authors) > 3 else ''}"
            )
